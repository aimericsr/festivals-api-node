...
class PerformingEventController {
    async addPerformingEvent(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            const pool = req.pool;
            const rowCreated = await performingEventService.addPerformingEvent(artist_id, event_id, pool, next);
    ...
    async delPerformingEvent(req: Request, res: Response, next: NextFunction) {
        try {
            const {artist_id, event_id} = req.body;
            const pool = req.pool;
            const rowDeleted = await performingEventService.delPerformingEvent(artist_id, event_id, pool, next);
